...
def main(config, logger, exp_dir, args):
    ...
    if args.cuda == 'cpu':
        ...
        # spatial_unet = Guide_UNet(config)
    else:
        temporal_unet = Temporal_UNet(config).cuda()
        # spatial_unet = Guide_UNet(config).cuda()
    # print(unet)
    ...
    mask_traj = np.swapaxes(mask_traj, 1, 2)

    mask_dataset = TensorDataset(mask_traj)
    ...
    # optimizer
    t_optim = torch.optim.AdamW(temporal_unet.parameters(), lr=lr)  # Optimizer
    ...
    for epoch in range(1, config.training.n_epochs + 1):
        ...
        for _, (mask_traj) in enumerate(mask_dataloader):
            mask_traj = mask_traj[0]
            if args.cuda == 'cpu':
                x0 = mask_traj
            ...
            pred_noise = temporal_unet(xt.float(), t)
        ...
        if (epoch) % 50 == 0:
            ...
            torch.save(temporal_unet.state_dict(), m_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--traj_path', type=str, default='data/TKY/train_set.csv')
    parser.add_argument('--gps_path', type=str, default='data/TKY/gps')
    parser.add_argument('--cat_path', type=str, default='data/TKY/category')
