...
from Transformer_encoder import CheckinTransformerEncoder
...
def Normalize(in_channels):
    return torch.nn.GroupNorm(num_groups=48,
...
class Model(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ch, out_ch, ch_mult = config.model.ch, config.model.mask_out_ch, tuple(
            config.model.ch_mult)
        num_res_blocks = config.model.num_res_blocks
        attn_resolutions = config.model.attn_resolutions
        dropout = config.model.dropout

        mask_in_channels = config.model.mask_in_channels
        ...
        self.in_channels = mask_in_channels
        ...
        # downsampling
        self.conv_in = torch.nn.Conv1d(mask_in_channels,
                                       self.ch,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
...
class Spatial_Model(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ch, out_ch, ch_mult = config.model.spatial_ch, config.model.spatial_out_ch, tuple(
            config.model.ch_mult)
        num_res_blocks = config.model.num_res_blocks
        attn_resolutions = config.model.attn_resolutions
        dropout = config.model.dropout

        mask_in_channels = config.model.spatial_in_channels
    ...
    def forward(self, x, t, extra_embed=None):
        ...
        # timestep embedding
        temb = get_timestep_embedding(t, embed_length)
        temb = self.temb.dense[0](temb)
        temb = nonlinearity(temb)
        temb = self.temb.dense[1](temb)
        if extra_embed is not None:
...
class Conditional_UNet(nn.Module):
    ...
    def forward(self, x, t, end_gps, h, current_t):
        place_vector = self.place_emb(end_gps)
        guide_vector = self.guide_emb(h)
        current_t = get_timestep_embedding(current_t, self.ch)
        current_t = self.unet.temb.dense[0](current_t)
        current_t = nonlinearity(current_t)
        current_t = self.unet.temb.dense[1](current_t)
        extra_embed = place_vector + guide_vector + current_t
        return self.unet(x, t, extra_embed)
...
class Spatial_UNet(nn.Module):
    def __init__(self, config):
        super(Spatial_UNet, self).__init__()
        self.config = config
        self.attr_dim = config.model.attr_dim
        self.guidance_scale = config.model.guidance_scale
        self.unet = Spatial_Model(config)
        self.ch = config.model.ch * 4
